...
{		
	...
	if ((align & -align) != align) return NULL;
...
i386_vm_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// 
	// size = -KERNBASE = -(f0000000) = 10000000 = 2^32 - KERNBASE
	boot_map_segment(boot_pgdir, KERNBASE, -KERNBASE, 0x0, (PTE_W|PTE_P));
...
page_init(void)
{
	...
	// Physical page 0 is marked as being in use to preserve the
	// real-mode IDT and BIOS structures in case we ever need them.
	pages[0].pp_ref = 1;
	...
	// Physical addresses ranging from 0x000a0000 to 0x000fffff are
	// usually reserved to BIOS routines nd to map the internal 
	// memory of ISA graphic cards. This is the well known hole from
	// 640 KB to 1 MB in all IBM-compatible PCs. The physical addresses
	// exist but they are reserved, and the corresponding page frames
	// cannot be used by the OS. We mark them as being in use.
	for (i = (IOPHYSMEM/PGSIZE); i < (EXTPHYSMEM/PGSIZE); ++i)
		pages[i].pp_ref = 1;		

	// Some pages in extended memory are already in use. boot_freemem
	// points to the next byte of free memory so we need to mark page 
	// frames up to this point as being used.
...
{
	// Do we have any free pages to allocate?
	if (!LIST_EMPTY(&page_free_list)) {

		// Get a pointer to the first free page and remove it 
		// from the free list to allocate.
	...
	else return -E_NO_MEM; // Out of memory!
}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	// Decompose the 32 bits of a the linear address to the fields we need.
	uintptr_t pdx = PDX(va); // Directory field
	uintptr_t ptx = PTX(va); // Table field

	// The Directory field is the index into the page directory that determines 
	// the entry that points to the proper page table. This entry contains the 
	// physical address of the Page Table.
	if (pgdir[pdx] & PTE_P) {

		// The Table field is an index into the page table that determines the 
		// entry containing the physical address of the page frame containing 
		// the page. We want to return a pointer to the this page table entry.
		ptep = KADDR(PTE_ADDR(pgdir[pdx]));
		ptep += ptx;
	}
	else if (create && (page_alloc(&pp) == 0)) {

		// Set ref count and insert the newly allocated page table (physical addr).
		pp->pp_ref = 1;
		pgdir[pdx] = page2pa(pp)|PTE_U|PTE_W|PTE_P;

		// Clear the new page table ensuring no pages are present.
		memset(KADDR(PTE_ADDR(pgdir[pdx])), 0x0, PGSIZE);
		
		// Get a pointer into the new page table to the entry we want.
		ptep = KADDR(PTE_ADDR(pgdir[pdx]));
		ptep += ptx;
	}
	return ptep;
...
{
	// Get a pointer to the page table entry associated with va and
	// allocate a new page table if it doesn't already exist.
	pte_t *ptep = pgdir_walk(pgdir, va, 1);

	if (ptep) {
		physaddr_t pa = page2pa(pp);

		// If there's already a page mapped and it's not the same as 
		// the pp we wish to map, remove it. By calling page_remove()
		// we also take care of invalidating the TLB.
		if ((*ptep & PTE_P) && (PTE_ADDR(*ptep) != pa))			
			page_remove(pgdir, va);

		// If page was not already mapped increment the ref count and map
		// the physical page to the page at the virtual address.
		if (!(*ptep)) ++pp->pp_ref;
		*ptep = pa|perm|PTE_P;
	...
	else return -E_NO_MEM; // Page table couldn't be allocated
}
...
boot_map_segment(pde_t *pgdir, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	for (i = 0; i < size; i += PGSIZE) {
		// Find the page table entry for each linear address
		// If it doesn't exist then we try to create one.
		void *va = (void *) (la+i);
		pte_t *ptep = pgdir_walk(pgdir, va, 1);

		// Map the linear address to the physical.
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	// Get the page table entry mapped at virtual address va
	// Do NOT create if no page is mapped
	pte_t *ptep = pgdir_walk(pgdir, va, 0);

	if (ptep) { // We found the page
		// Store it?
		if (pte_store) *pte_store = ptep;

		// Return a pointer to the page given its physical address.
		pp = pa2page(PTE_ADDR(*ptep));
	}
	return pp; // There is no page mapped at va
}
...
{
	// Lookup the page mapped at virtual address va and
	// store the address of the page table entry for it
	pte_t *ptep = NULL;
	struct Page *pp = page_lookup(pgdir, va, &ptep);

	if (pp) { // We found a page at the given address
		page_decref(pp);
		*ptep = 0;
		tlb_invalidate(pgdir, va);
	}

	// No page found, silently do nothing...
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	// Get a page aligned bounds on the range of memory to check.
	addr = (void *) ROUNDDOWN(va, PGSIZE);
  len = ROUNDUP(len, PGSIZE);

	// Iterate through the range of memory checking that the environment 
	// is allowed to access it. We perform several tests here.
	for (i = 0; i < len; i += PGSIZE) {

		// Is the address is below ULIM?
		if ((void *) ULIM < addr+i) {
			user_mem_check_addr = (uintptr_t) addr+i;
			return -E_FAULT;
		}

		// Get the page table entry for this address.
		ptep = pgdir_walk(env->env_pgdir, addr+i, 0);

		// Is the page table mapped?
		if ((!ptep) || (!(*ptep & PTE_P))) {
		...
		if (((perm & PTE_U) && !(*ptep & PTE_U)) || 
				((perm & PTE_W) && !(*ptep & PTE_W))) {
			user_mem_check_addr = (uintptr_t) va+i;
			return -E_FAULT;
		}
	}

	// Passed all tests, the user program can access this range of addresses.
	return 0;
}
